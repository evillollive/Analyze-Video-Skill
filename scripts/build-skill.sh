...
#
# Produces dist/analyze-video.skill, a zip with a single top-level
# `analyze-video/` directory containing SKILL.md and the scripts/ runtime.
# claude.ai's skill upload has a 200-file cap; `export-ignore` in
# .gitattributes + the zip -d strips below keep the bundle lean.
set -euo pipefail
...
mkdir -p dist
OUT="dist/analyze-video.skill"
git archive --format=zip --prefix=analyze-video/ --output="$OUT" HEAD
...
zip -d "$OUT" \
  "analyze-video/hooks/*" \
  "analyze-video/commands/*" \
  "analyze-video/.claude-plugin/*" \
  "analyze-video/.codex-plugin/*" \
  > /dev/null 2>&1 || true
