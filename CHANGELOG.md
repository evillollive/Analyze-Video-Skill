...
- **Re-runs re-tiled contact sheets for chunks that were already finished.** `extract` resumes a completed frame directory for free — it fingerprints its inputs and reuses the matching signature subdir — but the contact sheet built from those frames had no equivalent check, so `_process_chunk` re-ran the ffmpeg tile pass unconditionally. The documented recovery path for a runner timeout or an interrupted run is "re-run the exact same command", which meant every already-extracted chunk still paid a full decode-and-re-encode of its frames to reproduce a sheet that was already on disk, byte for byte.

  A sheet is now recorded alongside a `contact_sheet.jpg.sheet.json` sidecar holding the extraction signature of the frames it was tiled from plus the tiling options (`cols`, tile width, quality, frame count). A later run reuses the sheet only when that signature matches exactly, so re-extracted, trimmed, or differently-tiled frames always re-tile. The sidecar is deleted *before* ffmpeg runs, so a sheet left half-written by an interrupted run can never look valid. `--force` re-tiles and overwrites; sidecar reads and writes are best-effort, so a read-only or permission-restricted sandbox simply falls back to tiling as before.
