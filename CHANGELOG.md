...
- **Contact sheets re-tiled on every resumed run.** Frame extraction already resumes: `extract()` reuses a signature-keyed `frames/<sig>/` directory when the inputs are unchanged, so a re-run after a timeout skips straight past the chunks it already finished. The contact sheet did not participate. `make_contact_sheet()` ran unconditionally, so every resumed run still spawned an ffmpeg process per chunk to decode and re-tile up to 120 JPEGs into a sheet byte-identical to the one already sitting next to them. On an auto-chunked long video that is one wasted full-frame-set decode per chunk, and on the resume path (where extraction itself is nearly free) it was essentially the entire remaining cost of the run.

  A sheet is now fingerprinted by the frames directory it was tiled from, the frame count, and the tiling parameters (`cols`, `tile_width`, `quality`), recorded in a `.contact_sheet.jpg.sheet.json` marker beside it. A later run with a matching fingerprint reuses the sheet instead of re-tiling; a measured re-run dropped from ~200 ms to ~3 ms per sheet with byte-identical output. Because the frames directory name is itself the extraction signature, any change to fps, resolution, range, or the source video lands on a different directory and therefore a different sheet fingerprint, so a stale sheet can never be served for new frames. The marker is deleted before tiling and written only after ffmpeg reports success and the output is non-empty, so a run killed mid-tile leaves nothing vouching for a truncated sheet; a missing or zero-byte sheet re-tiles regardless of the marker. `--force` re-tiles unconditionally, and marker read/write failures (read-only sandboxes) just fall back to re-tiling as before.
