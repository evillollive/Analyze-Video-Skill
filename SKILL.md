...
`process.py` is resumable. Re-running with the same `--source` and `--out-dir` reuses any chunk whose frames are still valid (matched by an extraction signature), so an interrupted long video continues instead of restarting from zero. Each distinct set of extraction settings writes into its own `frames/<sig>/` subfolder, so a re-run never has to delete a previous run's files (which some sandboxes forbid) and stale frames can't pollute the result. Contact sheets are reused on the same terms: a sheet whose frames and tiling settings are unchanged is not re-tiled, so a resumed run skips a full ffmpeg pass per chunk. If a run is killed, check `status.json` to see where it stopped, then just re-run the same command. Pass `--force` to ignore cached output and re-download + re-extract everything.
...
- **Interrupted download**: re-run the exact same command. A download killed partway through resumes from the bytes already on disk rather than starting over, so a timed-out or dropped transfer does not re-fetch the whole video. Use `--force` only when the partial data should be discarded.
- **Download blocked by login, age gate, bot check, members-only, or private access**: explain the specific access issue. For public YouTube URLs, `download.py` tries player clients in order — android first (it bypasses YouTube's n-challenge without a JavaScript runtime and avoids the 403s the web client hits from server/cloud IPs), then ios (a separate extraction path that often works when android is bot-flagged or 429-rate-limited), then the default web client. Every attempt runs with `--ignore-no-formats-error` so that when a client's media formats are gated behind a PO token, yt-dlp still writes the subtitle tracks it located instead of aborting with "Requested format is not available". If access still fails and the user can view the video and authorizes it, retry with `--cookies-from-browser <browser>` or `--cookies <file>`. Otherwise ask for a local file. Note: `--cookies-from-browser` only works when yt-dlp runs on the SAME OS as the browser. In a Linux/cloud sandbox it cannot read a macOS/Windows browser's cookie store — and public YouTube itself is frequently bot-flagged (429) from datacenter IPs — so run yt-dlp host-side (e.g. via a Mac/Windows shell tool) for both cookie-based access and bot-check avoidance, then point `process.py` at the resulting local file.
